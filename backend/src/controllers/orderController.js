...
export const checkout = async (req, res) => {
  ...
  try {
    ...
    const singleVendor = cartItems.every((item) => String(item.vendor_id) === String(vendorId));
    if (!singleVendor) throw new Error("Only one vendor allowed per checkout");
    ...
    await connection.execute(
      ...
      [
        ...
        paymentMethod === "Cash on Delivery" ? null : `TX-${Date.now()}`
      ]
    );
    ...
    const orderRows = await query(`${orderSelect} WHERE o.id = ? LIMIT 1`, [orderResult.insertId]);
